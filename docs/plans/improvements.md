...
## Статус реализации (актуально для кода в репозитории)

| Область | Статус |
|---------|--------|
| Pipeline без ApeRAG, validator, extractor (docx / PyMuPDF / Vision Qwen), orchestrator | Реализовано |
| `EvaluatorConfig`, рубрики/промпты в БД, staff UI, per-config API `/api/ev/<slug>/…` | Реализовано |
| `SystemSettings`, валидация суммы `evaluation_slots` | Модель + `clean()`; **семафоры** по-прежнему из `.env` (`NITEC_MAX_WORKERS` и т.д.), не из БД |
| Доставка Beles / webhook после оценки | `tasks/delivery.py` |
| Диагностика `used_vision_ocr`, `used_fix_docx`, `was_empty_doc` | В модели и дашборде |
| Аудио / Whisper (`enable_audio`, `whisper_model`) | Поля в модели; **пайплайна нет** |
| Шифрование `beles_api_key` в БД | **Нет**, обычное поле |
| Полный перенос FastAPI-сервиса, Nginx, выключение старого сервиса | **Эксплуатация**, см. `docs/operations/deployment.md` |

Детали API и конфигурации: **`docs/reference/api.md`**, **`docs/reference/config-env.md`**.
