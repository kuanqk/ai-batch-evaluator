...
**Файл:** `pipeline/validator.py` (`fix_broken_docx`)
...
из-за чего LibreOffice/извлечение падают. Фиксируем in-place.  
Прямое чтение XML из docx — `pipeline/docx_utils.py` (`extract_text_from_docx_xml` и др.).
...
**Файл:** `tasks/evaluate.py` (синхронный ORM, не async SQL)

После `asyncio.run(run_pipeline(...))` результат пишется в `Evaluation` через Django ORM, счётчики job обновляются через `F()` + `_try_finalize_job`; при завершении job вызывается `_notify_webhook_sync` при наличии `webhook_url`. Успешная оценка с `EvaluatorConfig` может вызвать `tasks/delivery.deliver_evaluation_outcome`.
...
`process_file` в `tasks/evaluate.py` использует `bind=True` и большой `max_retries` в том числе для паузы батча (`JobPaused`). При ошибке pipeline — `failed`, инкремент счётчика job.

Ручная переобработка: `POST /api/evaluations/<eval_id>/retry/`.

---

## Webhook уведомление (при завершении батча)

Синхронная функция `_notify_webhook_sync(job_id)` в `tasks/evaluate.py` — POST при финализации job, если задан `webhook_url`.
